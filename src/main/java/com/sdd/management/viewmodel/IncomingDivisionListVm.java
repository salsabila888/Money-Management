...
public class IncomingDivisionListVm {
				...
				public void render(Row row, final Tpiloting data, int index) throws Exception {
					...
					btnEdit.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
						...
						public void onEvent(Event event) throws Exception {
							...
							win = (Window) Executions.createComponents("/view/inventory/incomingdivisionentry.zul", null,
									map);
							win.setWidth("70%");
							...
							win.addEventListener(Events.ON_CLOSE, new EventListener<Event>() {
								...
								public void onEvent(Event event) throws Exception {
									doReset();
									BindUtils.postNotifyChange(null, null, IncomingDivisionListVm.this, "*");
								}
							});
						}
					});
	...
	public void doExport() {
		try {
			if (objList != null && objList.size() > 0) {
				...
				cell.setCellValue("DAFTAR PERSEDIAAN INTEGRASI PFA");
				rownum++;
				...
				datamap.put(1,
						new Object[] { "No", "Nama Produk", "Lokasi", "Kode Outlet", "Prefix", "No Seri Awal",
								"No Seri Akhir", "Jumlah Lembar/Buku", "Harga", "Status", "No SPK", "Tanggal SPK", 
								"No PKS", "Tanggal PKS", "No Surat Vendor", "Tanggal Surat Vendor", "Tahun Produksi",
								"Vendor", "Memo", "Diinput Oleh", "Disetujui Oleh", "Tgl Input" });
				no = 2;
				for (Tpiloting data : objList) {
					datamap.put(no,
							new Object[] { no - 1, data.getProducttype(), data.getBranchname(), data.getOutlet(),
									...
									"Rp." + NumberFormat.getNumberInstance().format(data.getItemprice()),
									AppData.getStatusPilotingLabel(data.getStatus()), 
									data.getSpkno(), dateLocalFormatter.format(data.getSpkdate()),
									data.getPksno(), dateLocalFormatter.format(data.getPksdate()),
									data.getVendorletterno(), dateLocalFormatter.format(data.getVendorletterdate()),
									data.getManufacturedate(), data.getMsupplier().getSuppliername(), 
									data.getMemo(), data.getInsertedby(),
