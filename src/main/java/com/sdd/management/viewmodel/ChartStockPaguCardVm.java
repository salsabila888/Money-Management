...
public class ChartStockPaguCardVm {
				...
				if (data.getA() != 0) {
					...
					agt6.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
						...
						public void onEvent(Event arg0) throws Exception {
							...
							map.put("title", "Jumlah Stock VS Pagu Diatas 50% untuk Org " + data.getDescription());

							Window win = (Window) Executions.createComponents("/view/dashboard/stockpagucarddetail.zul",
									null, map);
					...
					row.getChildren()
							.add(new Label(data.getA() != null ? NumberFormat.getInstance().format(data.getA()) : "0"));
				}
				if (data.getB() != 0) {
					...
					row.getChildren()
							.add(new Label(data.getB() != null ? NumberFormat.getInstance().format(data.getB()) : "0"));
				}
				if (data.getC() != 0) {
					...
					alt3.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
						...
						public void onEvent(Event arg0) throws Exception {
							...
							map.put("title", "Jumlah Stock VS Pagu Dibawah 25% untuk Org " + data.getDescription());
					...
					row.getChildren()
							.add(new Label(data.getC() != null ? NumberFormat.getInstance().format(data.getC()) : "0"));
				}
			}
		});
	}
	...
	public void doSearch() {
		if (date != null) {
			try {
				 ...
				 * 
				 * for (int i = 0; i < catorg.length; i++) { Vpersodelivdata obj = new
				 * Vpersodelivdata(); obj.setOrg(catorg[i]);
				 * obj.setDescription(catdescription[i]);
				 * obj.setTotalperso(catperso[i].intValue());
				 * obj.setTotaldeliv(catorder[i].intValue()); objList.add(obj); }
	...
	/*
	 * private void initSeries(Charts chart) throws Exception { try {
	 * List<Vpersodeliv> objList =
	 * oDao.listOrdOnOrdVsProd(dateFormatter.format(date)); catperso = new
	 * Double[objList.size()]; int i = 0; totalperso = 0; for (Vpersodeliv obj :
	 * objList) { catperso[i++] = new Double(obj.getTotaldata()); totalperso +=
	 * obj.getTotaldata(); } Series ord = new Series("Order"); ord.setName("Order");
	 * ord.setType("column"); ord.setYAxis(1); ord.setData(catperso);
	 * chart.addSeries(ord); } catch (Exception e) { e.printStackTrace(); }
	 * 
	 * try { List<Vpersodeliv> objList =
	 * oDao.listProdOnOrdVsProd(dateFormatter.format(date)); catorder = new
	 * Double[objList.size()]; int i = 0; totaldeliv = 0; for (Vpersodeliv obj :
	 * objList) { catorder[i++] = new Double(obj.getTotaldata()); totaldeliv +=
	 * obj.getTotaldata(); } Series prod = new Series("Produksi");
	 * prod.setName("Produksi"); prod.setType("column"); prod.setYAxis(2);
	 * prod.setData(catorder); chart.addSeries(prod); } catch (Exception e) {
	 * e.printStackTrace(); }
	 * 
