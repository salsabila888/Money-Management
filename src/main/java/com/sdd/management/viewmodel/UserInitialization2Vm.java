...
public class UserInitialization2Vm {
						...
						public void onEvent(Event event) throws Exception {
							...
							if (mapNotif.get(obj.getMmenu().getMmenupk()) != null) {
								try {
									...
									List<Tnotif> notifDelList = new TnotifDAO()
											.listByFilter("mmenufk = " + obj.getMmenu().getMmenupk(), "mmenufk");
									for (Tnotif n : notifDelList) {
										new TnotifDAO().delete(session, n);
									}
									transaction.commit();
									session.close();
									...
									// -------MENGURANGI JUMLAH NOTIF PADA MENU NAME----------
									if (!obj.getMmenu().getMenugroup().trim()
											.equals(obj.getMmenu().getMenusubgroup().trim())) {
										isSubgroup = true;
									}

									Component comp = event.getTarget().getFirstChild(); // Hlayout
									for (Component child : comp.getChildren()) {
										if (child instanceof Label) {
											((Label) child).setStyle("font-size:16px");
										}
									...
									// -------MENGURANGI JUMLAH NOTIF JIKA ADA MENU SUBGROUP--------
									if (isSubgroup) {
										int totalMenugroup = 0;
										int totalMenusubgroup = 0;

										Component compParent = event.getTarget().getParent().getParent().getParent()
												.getParent().getParent().getParent();
										if (compParent instanceof Treeitem) {
											Component compFirst = compParent.getFirstChild().getFirstChild();
											if (compFirst instanceof Treecell) {
												totalMenugroup = mapTotalnotif.get(obj.getMmenu().getMenugroup())
														- mapTotalnotif.get(obj.getMmenu().getMenuname());
												mapTotalnotif.put(obj.getMmenu().getMenugroup(), totalMenugroup);
												for (Component child : compFirst.getFirstChild().getChildren()) {
													if (totalMenugroup == 0) {
														if (child instanceof Label) {
															((Label) child).setStyle("font-size:16px");
														}
														...
														if (child instanceof Div) {
															compFirst.getFirstChild().removeChild(child);
														}
													} else {
														if (child instanceof Div) {
															Component childTotal = child.getFirstChild();
															((Label) childTotal).setValue(" " + totalMenugroup);
															;
														}
													}
												}
										...
										Component compParentTwo = event.getTarget().getParent().getParent().getParent()
												.getParent();
										if (compParentTwo instanceof Treeitem) {
											Component compFirst = compParentTwo.getFirstChild().getFirstChild();
											if (compFirst instanceof Treecell) {
												totalMenusubgroup = mapTotalnotif.get(obj.getMmenu().getMenusubgroup())
														- mapTotalnotif.get(obj.getMmenu().getMenuname());
												mapTotalnotif.put(obj.getMmenu().getMenusubgroup(), totalMenusubgroup);
												for (Component child : compFirst.getFirstChild().getChildren()) {
													if (totalMenusubgroup == 0) {
														if (child instanceof Label) {
															((Label) child).setStyle("font-size:16px");
														}
														...
														if (child instanceof Div) {
															Component childTotal = child.getFirstChild();
															((Label) childTotal).setValue(" " + totalMenusubgroup);
															;
														}
													}
												}
										...
										// -----MENGURANGI JUMLAH NOTIF JIKA TIDAK ADA MENU SUBGROUP------
										Component compParent = event.getTarget().getParent().getParent().getParent()
												.getParent();
										if (compParent instanceof Treeitem) {
											Component compFirst = compParent.getFirstChild().getFirstChild();
											if (compFirst instanceof Treecell) {
												int total = 0;
												if (isSubgroup) {
													total = mapTotalnotif.get(obj.getMmenu().getMenusubgroup())
															- mapTotalnotif.get(obj.getMmenu().getMenuname());
													mapTotalnotif.put(obj.getMmenu().getMenusubgroup(), total);
												} else {
													total = mapTotalnotif.get(obj.getMmenu().getMenugroup())
															- mapTotalnotif.get(obj.getMmenu().getMenuname());
													mapTotalnotif.put(obj.getMmenu().getMenugroup(), total);
												}
												for (Component child : compFirst.getFirstChild().getChildren()) {
													if (total == 0) {
														if (child instanceof Label) {
														...
														if (child instanceof Div) {
															Component childTotal = child.getFirstChild();
															((Label) childTotal).setValue(" " + total);
															;
														}
													}
												}
							...
							// -------------------------------------------------------------------------------------
							System.out.println(obj.getMmenu().getMenupath());
