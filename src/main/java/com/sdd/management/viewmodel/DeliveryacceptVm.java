	...
	public void doRender() {
		try {
			if (oUser.getMbranch().getBranchlevel() == 1) {
				...
				filter = "tglterima is null and penerima is null and tdelivery.mbranchfk = " + oUser.getMbranch().getMbranchpk()
						+ " and tdelivery.status = '" + AppUtils.STATUS_DELIVERY_DELIVERY + "' ";
			}
			...
			List<Mproductgroup> objList = new MproductgroupDAO().listByFilter(filterbranch, "mproductgrouppk");
			for (Mproductgroup obj : objList) {
				List<Vsumbyproductgroup> productList = new TdeliveryDAO().getSumdataByProductgroup(
				...
				btn.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
					...
					public void onEvent(Event event) throws Exception {
						...
						Executions.createComponents("/view/delivery/deliveryacceptedentry.zul", divContent, map);
					}
				});
