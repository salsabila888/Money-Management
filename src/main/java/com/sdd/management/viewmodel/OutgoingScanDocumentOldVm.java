...
public class OutgoingScanDocumentOldVm {
	...
	private List<Tbranchstockitem> objBranchitemList = new ArrayList<>();
	...
	public void getStartno() {
		try {
			if (branchlevel > 1) {
				...
				if (objStock != null) {
					branchstockitem = new TbranchstockitemDAO().listNativeByFilter("tbranchstockfk = "
							+ objStock.getTbranchstockpk() + " and status = '" + AppUtils.STATUS_SERIALNO_ENTRY + "'",
							"numerator");
					if (branchstockitem.size() > 0) {
						itemno = String.valueOf(branchstockitem.get(0).getNumerator());
						itemnoend = String.valueOf(Integer.parseInt(itemno) + obj.getItemqty() - 1);
					} else {
						Messagebox.show(
	...
	public void doReset() {
		...
		objBranchitemList = new ArrayList<Tbranchstockitem>();
		refresh();
	}
	...
	public void refresh() {
		if (branchlevel > 1)
			grid.setModel(new ListModelList<Tbranchstockitem>(objBranchitemList));
		else
	...
	public void doAdd() {
		if (outstanding <= 0) {
			Messagebox.show(
					"Tidak bisa menambah data karna jumlah data yang dimasukkan sudah setara atau melebihi outstanding.",
					"Info", Messagebox.OK, Messagebox.INFORMATION);
		} else {
			btnAdd.setDisabled(true);
			tr = new Tr();
...
//				if (txtAwal != null)
//					txtAkhir.setValue(itemnoend);
//			} else {
//				if (txtAwal != null) {
//					String newitemendno = String.valueOf(Integer.parseInt(itemno) + obj.getItemqty() - 1);
//					txtAkhir.setValue(newitemendno);
//				} else {
//					txtAkhir.setValue(null);
//				}
//			}
			...
			btn.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
				...
				public void onEvent(Event event) throws Exception {
					if (txtPrefix == null || txtPrefix.getValue().trim().length() == 0)
						...
						Messagebox.show("Silahkan isi data nomor seri awal dengan data numerik.", "Info", Messagebox.OK,
						...
						if (isValid) {
							...
							refresh();
							BindUtils.postNotifyChange(null, null, OutgoingScanDocumentOldVm.this, "outstanding");
						} else {
		...
		try {
			...
			if (branchlevel == 1) {
				...
				if (securitiesitemList.size() > 0) {
					...
					item.setItemendno(Integer.parseInt(endno.trim()));
				...
				if (objStock != null) {
					branchstockitem = new TbranchstockitemDAO().listNativeByFilter("tbranchstockfk = "
							+ objStock.getTbranchstockpk() + " and status = '" + AppUtils.STATUS_SERIALNO_ENTRY
							+ "' and numerator between " + startno.trim() + " and " + endno.trim(), "numerator");
					if (branchstockitem.size() > 0) {
						for (Tbranchstockitem data : branchstockitem) {
							if (!listData.contains(data.getItemno().trim())) {
								if (data.getItemno().trim()
										.equals(prefix.trim().toUpperCase() + data.getNumerator().toString())) {
									if (outstanding > 0) {
										objBranchitemList.add(data);
										listData.add(data.getItemno().trim());
										outstanding = outstanding - 1;
									}
								}
				...
				if (objStock != null) {
					branchstockitem = new TbranchstockitemDAO().listByFilter("tbranchstockfk = "
							+ objStock.getTbranchstockpk() + " and numerator between " + startno.trim() + " and "
							+ endno.trim() + " and status = '" + AppUtils.STATUS_SERIALNO_ENTRY + "'", "numerator");
					if (branchstockitem.size() > 0) {
						for (Tbranchstockitem data : branchstockitem) {
							if (!listData.contains(data.getItemno().trim())) {
								if (data.getItemno().trim()
	...
	public void doProcess() {
		try {
			...
			} else if (objBranchitemList.size() > 0) {
				try {
					session = StoreHibernateUtil.openSession();
					transaction = session.beginTransaction();
					for (Tbranchstockitem data : objBranchitemList) {
						data.setStatus(AppUtils.STATUS_SERIALNO_OUTINVENTORY);
		...
		try {
			Mmenu mmenu = new MmenuDAO()
					.findByFilter("menupath = '/view/inventory/outgoing.zul' and menuparamvalue = 'list'");
