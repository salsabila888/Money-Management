...
public class MpaymentVm {
	...
	private MpaymentListModel model;
	private MpaymentDAO oDao = new MpaymentDAO();
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		if (listbox != null) {
			listbox.setItemRenderer(new ListitemRenderer<Mpayment>() {

				@Override
				public void render(Listitem item, final Mpayment data, int index) throws Exception {
					Listcell cell = new Listcell(String.valueOf((SysUtils.PAGESIZE * pageStartNumber) + index + 1));
					item.appendChild(cell);
					cell = new Listcell(data.getPaymenttype());
					item.appendChild(cell);
				}
			});
		}
	...
	public void refreshModel(int activePage) {
		...
		model = new MpaymentListModel(activePage, SysUtils.PAGESIZE, filter, orderby);
		if (needsPageUpdate) {
...
//		}
						...
						public void onEvent(Event event) throws Exception {
							if (event.getName().equals("onOK")) {
								try {
									...
									needsPageUpdate = true;
									doReset();
									BindUtils.postNotifyChange(null, null, MpaymentVm.this, "objForm");
								} catch (HibernateException e) {
									transaction.rollback();
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {
				try {
					String paymenttype = (String) ctx.getProperties("paymenttype")[0].getValue();
					
					if (paymenttype == null || "".equals(paymenttype.trim()))
						this.addInvalidMessage(ctx, "paymenttype", Labels.getLabel("common.validator.empty"));
	...
	public Mpayment getObjForm() {
		return objForm;
	}

	public void setObjForm(Mpayment objForm) {
