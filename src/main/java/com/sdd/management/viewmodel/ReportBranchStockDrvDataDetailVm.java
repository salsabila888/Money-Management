...
public class ReportBranchStockDrvDataDetailVm {
	...
	private List<Tderivatifdata> objList = new ArrayList<>();
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view, @ExecutionArgParam("obj") Tbranchstock obj)
		...
		grid.setRowRenderer(new RowRenderer<Tderivatifdata>() {
			...
			public void render(Row row, final Tderivatifdata data, int index) throws Exception {
				...
				row.getChildren()
						.add(new Label(data.getTembossdata().getMproduct().getMproducttype().getProducttype()));
				row.getChildren().add(new Label(data.getTembossdata().getProductcode()));
				row.getChildren()
						.add(new Label(data.getTembossdata().getMproduct() != null
								? data.getTembossdata().getMproduct().getProductname()
								: ""));
				row.getChildren().add(new Label(data.getTembossdata().getBranchname()));
				row.getChildren().add(new Label(data.getTembossdata().getBranchid()));
				row.getChildren().add(new Label(data.getTembossdata().getKlncode()));
			}
		});
		...
		try {
			if (objList != null && objList.size() > 0) {
				...
				for (Tderivatifdata data : objList) {
					datamap.put(no, new Object[] { no - 1, data.getCardno(),
							datelocalFormatter.format(data.getTembossdata().getOrderdate()),
							data.getTembossdata().getMproduct().getMproducttype().getProducttype(),
							data.getTembossdata().getProductcode(),
							data.getTembossdata().getMproduct().getProductname(),
							data.getTembossdata().getMbranch().getBranchname(),
							data.getTembossdata().getBranchid() != null ? data.getTembossdata().getBranchid() : "",
	...
	public void refreshModel() {
		try {
			orderby = "tderivatifdatapk desc";
			objList = new TderivatifdataDAO().dataStockList(filter, orderby);
	...
	public void doSearch() {
		filter = "tembossdata.mbranchfk = " + obj.getMbranch().getMbranchpk() + " and tembossdata.mproductfk = "
				...
				+ " and tembossdata.isactivated is null and tderivatif.status in (14, 15)";
