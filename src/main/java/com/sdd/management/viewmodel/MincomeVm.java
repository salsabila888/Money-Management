...
public class MincomeVm {
	...
	private MincomeListModel model;
	private MincomeDAO oDao = new MincomeDAO();
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		if (listbox != null) {
			listbox.setItemRenderer(new ListitemRenderer<Mincome>() {

				@Override
				public void render(Listitem item, final Mincome data, int index) throws Exception {
					Listcell cell = new Listcell(String.valueOf((SysUtils.PAGESIZE * pageStartNumber) + index + 1));
					item.appendChild(cell);
					cell = new Listcell(data.getIncomesource());
					item.appendChild(cell);
				}
			});
		}
	...
	public void refreshModel(int activePage) {
		...
		model = new MincomeListModel(activePage, SysUtils.PAGESIZE, filter, orderby);
		if (needsPageUpdate) {
...
//		}
						...
						public void onEvent(Event event) throws Exception {
							if (event.getName().equals("onOK")) {
								try {
									...
									needsPageUpdate = true;
									doReset();
									BindUtils.postNotifyChange(null, null, MincomeVm.this, "objForm");
								} catch (HibernateException e) {
									transaction.rollback();
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {
				try {
					String incomesource = (String) ctx.getProperties("incomesource")[0].getValue();
					
					if (incomesource == null || "".equals(incomesource.trim()))
						this.addInvalidMessage(ctx, "incomesource", Labels.getLabel("common.validator.empty"));
	...
	public Mincome getObjForm() {
		return objForm;
	}

	public void setObjForm(Mincome objForm) {
