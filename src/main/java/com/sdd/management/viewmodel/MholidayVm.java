...
import com.sdd.caption.model.MholidayListModel;
import com.sdd.utils.SysUtils;
import com.sdd.utils.db.StoreHibernateUtil;

public class MholidayVm {
	...
	private MholidayListModel model;
	private MholidayDAO oDao = new MholidayDAO();
	...
	private Mholiday objForm;
	private Date holiday;
	
	private SimpleDateFormat dateFormatter = new SimpleDateFormat("yyyy-MM-dd");
	private SimpleDateFormat datelcoalFormatter = new SimpleDateFormat("dd-MM-yyyy");
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		if (listbox != null) {
			listbox.setItemRenderer(new ListitemRenderer<Mholiday>() {

				@Override
				public void render(Listitem item, final Mholiday data, int index)
						throws Exception {
					Listcell cell = new Listcell(String.valueOf((SysUtils.PAGESIZE * pageStartNumber) + index + 1));
					item.appendChild(cell);
					cell = new Listcell(datelcoalFormatter.format(data.getHoliday()));
					item.appendChild(cell);
	...
	public void refreshModel(int activePage) {
		...
		model = new MholidayListModel(activePage, SysUtils.PAGESIZE, filter, orderby);
		if (needsPageUpdate) {					
	...
	public void doSearch() {
		...
		if (holiday != null) {
			...
			filter += "holiday = '" + dateFormatter.format(holiday) + "'";
		}	
		needsPageUpdate = true;
				...
				public void onEvent(Event event)
					...
					if (event.getName().equals("onOK")) {
						try {
							...
							needsPageUpdate = true;
							doReset();
							BindUtils.postNotifyChange(null, null, MholidayVm.this, "objForm");
						} catch (HibernateException e) {	
							transaction.rollback();
							Messagebox.show("Error : " + e.getMessage(), "Error", Messagebox.OK, Messagebox.ERROR);
							e.printStackTrace();
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {			
				Date holiday = (Date) ctx.getProperties("holiday")[0]
						.getValue();	
				if (holiday == null) 
					this.addInvalidMessage(ctx, "holiday",
							Labels.getLabel("common.validator.empty"));				
			}
		};
	}
	
	public Mholiday getObjForm() {
		return objForm;
	}

	public void setObjForm(Mholiday objForm) {
