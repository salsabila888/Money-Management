...
public class ReturEntryDocumentVm {
				...
				if (txtPrefix == null || txtPrefix.getValue().trim().length() == 0)
					...
					if (mproduct != null) {
						...
						if (isValid) {
							...
							doAdd();
							refresh();
							BindUtils.postNotifyChange(null, null, ReturEntryDocumentVm.this, "outstanding");
						} else {
		...
		if (inList.size() > 0) {
			try {
				...
				if (oUser.getMbranch().getBranchlevel() == 3) {
					...
					Mmenu mmenu = new MmenuDAO()
							.findByFilter("menupath = '/view/return/return.zul' and menuparamvalue = 'approvalwil'");
					NotifHandler.doNotif(mmenu, oUser.getMbranch(), objForm.getMproduct().getProductgroup(),
							oUser.getMbranch().getBranchlevel());
				}
	...
	public ListModelList<Mreturnreason> getMreturnreasonmodel() {
		...
		try {
			lm = new ListModelList<Mreturnreason>(
					AppData.getMreturnreason("productgroup = '" + arg + "' and isdestroy = 'N'"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return lm;
