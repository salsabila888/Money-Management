...
public class OutgoingScanEntryVm {
		...
		if (grid != null) {
			grid.setRowRenderer(new RowRenderer<Toutgoing>() {
				...
				public void render(Row row, final Toutgoing data, int index) throws Exception {
					...
					row.getChildren().add(
							new Label(data.getTorder() != null ? data.getTorder().getMbranch().getBranchname() : ""));
					...
					btnScan.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
						...
						public void onEvent(Event event) throws Exception {
							...
							if (data.getProductgroup().trim().equals(AppUtils.PRODUCTGROUP_PINPAD)) {
								...
								win = (Window) Executions.createComponents("/view/inventory/outgoingtokenupload.zul",
										null, map);
							} else {
		...
		try {
			if (year != null && month != null) {
				filter = "extract(year from toutgoing.entrytime) = " + year + " and "
						...
						+ productgroup + "' and Toutgoing.status = '" + AppUtils.STATUS_INVENTORY_OUTGOINGAPPROVED + "'";
	...
	public void doExport() {
		try {
			if (objList != null && objList.size() > 0) {
				...
				cell.setCellValue("DAFTAR OUTGOING " + AppData.getProductgroupLabel(productgroup));
				rownum++;
				...
				datamap.put(1, new Object[] { "No", "No Pemenuhan", "Jenis Produk", "Kode Produk",
						"Waktu Order", "Jumlah", "Status", "Requestor", "Pemutus" });
				no = 2;
				for (Toutgoing data : objList) {
					datamap.put(no, new Object[] { no - 1, data.getOutgoingid(),
							data.getMproduct().getMproducttype().getProducttype(), data.getMproduct().getProductcode(), datetimeLocalFormatter.format(data.getEntrytime()),
