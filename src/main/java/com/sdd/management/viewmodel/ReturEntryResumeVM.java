package com.sdd.caption.viewmodel;

import java.util.HashMap;
import java.util.Map;

import org.zkoss.bind.annotation.AfterCompose;
import org.zkoss.bind.annotation.Command;
import org.zkoss.bind.annotation.ContextParam;
import org.zkoss.bind.annotation.ContextType;
import org.zkoss.bind.annotation.ExecutionArgParam;
import org.zkoss.zk.ui.Component;
import org.zkoss.zk.ui.Executions;
import org.zkoss.zk.ui.select.Selectors;
import org.zkoss.zk.ui.select.annotation.Wire;
import org.zkoss.zul.Div;
import org.zkoss.zul.Window;

import com.sdd.caption.domain.Tincoming;
import com.sdd.caption.domain.Treturn;
import com.sdd.caption.utils.AppData;

public class ReturEntryResumeVM {
	
	private Treturn obj;
	
	private String arg;
	private String productgroupname;
	
	@Wire
	private Window winReturResume;

	@AfterCompose
		public void afterCompose(@ContextParam(ContextType.VIEW) Component view,
			@ExecutionArgParam("content") Div divContent, @ExecutionArgParam("arg") String arg,
			@ExecutionArgParam("obj") Treturn obj) {
		Selectors.wireComponents(view, this, false);
		
		this.obj = obj;
		this.arg = arg;
		
		productgroupname = AppData.getProductgroupLabel(obj.getMproduct().getProductgroup());
	}

	@Command
		public void doCreate() {
		Div divRoot = (Div) winReturResume.getParent();
		divRoot.getChildren().clear();
		Map<String, String> map = new HashMap<String, String>();
		map.put("arg", arg);
		Executions.createComponents("/view/return/returentrydocument.zul", divRoot, map);
	}
	
	@Command
		public void doList() {
		Div divRoot = (Div) winReturResume.getParent();
		divRoot.getChildren().clear();
		Map<String, String> map = new HashMap<String, String>();
		map.put("arg", arg);
		Executions.createComponents("/view/return/returproductlist.zul", divRoot, map);
	}

	public Treturn getObj() {
		return obj;
	}

	public void setObj(Treturn obj) {
		this.obj = obj;
	}
	
	public String getProductgroupname() {
		return productgroupname;
	}

	public void setProductgroupname(String productgroupname) {
		this.productgroupname = productgroupname;
	}

}
