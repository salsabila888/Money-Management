		...
		if (grid != null) {
			grid.setRowRenderer(new RowRenderer<Torderitem>() {
				...
				public void render(Row row, final Torderitem data, int index)
					...
					row.getChildren().add(new Label(AppData.getStatusLabel(data.getTtokenitem().getStatus())));
				}
			});
		}				
	}
		...
		try {
			if (filter.length() > 0) {
				...
				if (listData != null && listData.size() > 0) {
					...
					cell.setCellValue("DAFTAR ITEM PEMESANAN TOKEN");
					rownum++;
					...
					for (Torderitem data : listData) {
						datamap.put(no, new Object[] { no - 1, data.getItemno(), AppData.getStatusLabel(data.getTtokenitem().getStatus())});
						no++;
					}
