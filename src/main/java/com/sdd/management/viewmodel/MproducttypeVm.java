...
public class MproducttypeVm {
	...
	/*
	 * @Wire private Combobox cbProductorg;
	 */
	@Wire
		...
		if (listbox != null) {
			listbox.setItemRenderer(new ListitemRenderer<Mproducttype>() {
				...
				public void render(Listitem item, final Mproducttype data, int index) throws Exception {
					...
					item.appendChild(cell);
					cell = new Listcell(data.getCoano());
					item.appendChild(cell);
					cell = new Listcell(
	...
	public void save() {
		try {
			...
			if (data != null) {
				Messagebox.show("Gagal menambah tipe kartu, tipe kartu '" + objForm.getProducttype().trim()
						...
						public void onEvent(Event event) throws Exception {
							if (event.getName().equals("onOK")) {
								try {
									...
									needsPageUpdate = true;
									doReset();
									BindUtils.postNotifyChange(null, null, MproducttypeVm.this, "*");
								} catch (HibernateException e) {
									transaction.rollback();
