...
public class Chart2Vm {
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view,
			@ExecutionArgParam("isIndex") Boolean isIndex, @ExecutionArgParam("date") Date dateback)
					...
					if (data.getA() != 0) {
						...
						agt6.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
							...
							public void onEvent(Event arg0) throws Exception {
								...
								map.put("title", "Estimasi Ketersediaan Stock Diatas 6 Bulan untuk Jenis "
										+ data.getDescription());

								Window win = (Window) Executions
										.createComponents("/view/dashboard/chartstockestimatedetail.zul", null, map);
						...
						row.getChildren().add(
								new Label(data.getA() != null ? NumberFormat.getInstance().format(data.getA()) : "0"));
					}
					if (data.getB() != 0) {
						...
						agt3.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
							...
							public void onEvent(Event arg0) throws Exception {
								...
								map.put("title", "Estimasi Ketersediaan Stock antara 3 s/d 6 Bulan untuk Org "
										+ data.getDescription());
						...
						row.getChildren().add(
								new Label(data.getB() != null ? NumberFormat.getInstance().format(data.getB()) : "0"));
					}
					if (data.getC() != 0) {
						...
						alt3.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
							...
							public void onEvent(Event arg0) throws Exception {
								...
								map.put("title", "Estimasi Ketersediaan Stock Dibawah 3 Bulan untuk Org "
										+ data.getDescription());
						...
						row.getChildren().add(
								new Label(data.getC() != null ? NumberFormat.getInstance().format(data.getC()) : "0"));
					}
				}
			});
		}
		...
		if (oUser != null) {
			try {
				if (oDao.usermenuChecker("muserpk = " + oUser.getMuserpk()
						+ " and menupath = '/view/dashboard/chartstockestimate.zul'") > 0) {
