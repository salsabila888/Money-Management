...
public class EmbossUploadNewVm {
		...
		try {
			...
			if (media != null) {
				if (filename.substring(0, 1).equals("E") || filename.substring(0, 1).equals("A")) {
					...
					Messagebox.show("Penamaan file tidak sesuai dengan format nama file emboss", "Info", Messagebox.OK,
					...
					while ((line = reader.readLine()) != null) {
						try {
							...
							if (add1.trim().length() > 0)
								embossdata.setAddress1(add1);
							if (add2.trim().length() > 0)
								embossdata.setAddress2(add2);
							if (add3.trim().length() > 0)
								embossdata.setAddress3(add3);
							if (codepos.trim().length() > 0)
								embossdata.setZipcode(codepos);

							if (line.length() > 762) {
								accno = line.substring(743, 758).trim();
								embossdata.setAccno(accno);
								embossdata.setSeqno(line.substring(759, 765));
							} else {
						...
						public void onEvent(Event event) throws Exception {
							if (event.getName().equals("onOK")) {
								try {
									...
									doReset();
									BindUtils.postNotifyChange(null, null, EmbossUploadNewVm.this, "objForm");
								} catch (HibernateException e) {
									transaction.rollback();
